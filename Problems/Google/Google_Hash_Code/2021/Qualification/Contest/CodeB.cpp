...
public:
    ...
    vector<pair<int,int> > in_time; // time ve in id;
    Intersec() {
    }
    ...
    void basic_ans() {
        ...
        for(auto it: in_time) { 
            if(used.count(it.second)==0) {
                answer.push_back(MP(it.second,1));
                used.insert(it.second);
            }
        }
        set <int> used2;
        for(auto it: in_time) { 
            if(it.first<answer.size() && used2.count(it.second)==0) {
                int cp=0;
                for (int i = 0; i < answer.size(); i++)
                    if(answer[i].first==it.second)
                        cp = i;
                
                swap(answer[it.first], answer[cp]);
                used2.count(it.second);
            }
        }
    ...
    Service(string filename) { 
        ...
        for (int i = 0; i < numCars; i++)
        {
            ...
            for (int j = 0; j < P; j++)
            {
                ...
                if(j!=0)
                    curr_time+=st.streetsHelper[id].second;
                if(j!=P-1)
                    v[st.streetsHelper[id].first.second].in_time.push_back(MP(curr_time,id));
